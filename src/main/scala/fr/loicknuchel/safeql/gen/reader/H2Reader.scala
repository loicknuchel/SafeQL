...
class H2Reader(schema: Option[String] = None,
  ...
  protected[reader] def readConstraints(xa: doobie.Transactor[IO]): IO[List[Constraint]] =
    (fr0"SELECT CONSTRAINT_CATALOG, CONSTRAINT_SCHEMA, CONSTRAINT_NAME, CONSTRAINT_TYPE, TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME, UNIQUE_INDEX_NAME, CHECK_EXPRESSION, COLUMN_LIST, REMARKS, SQL, ID FROM INFORMATION_SCHEMA.CONSTRAINTS" ++
      schema.map(s => fr0" WHERE TABLE_SCHEMA=$s").getOrElse(fr0"") ++ fr0" ORDER BY TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME, COLUMN_LIST, CONSTRAINT_NAME").query[Constraint].to[List].transact(xa)

  protected[reader] def readCrossReferences(xa: doobie.Transactor[IO]): IO[List[CrossReference]] =
    (fr0"SELECT PKTABLE_CATALOG, PKTABLE_SCHEMA, PKTABLE_NAME, PKCOLUMN_NAME, FKTABLE_CATALOG, FKTABLE_SCHEMA, FKTABLE_NAME, FKCOLUMN_NAME, ORDINAL_POSITION, UPDATE_RULE, DELETE_RULE, FK_NAME, PK_NAME, DEFERRABILITY FROM INFORMATION_SCHEMA.CROSS_REFERENCES" ++
      schema.map(s => fr0" WHERE PKTABLE_SCHEMA=$s").getOrElse(fr0"") ++ fr0" ORDER BY PKTABLE_CATALOG, PKTABLE_SCHEMA, PKTABLE_NAME, PKCOLUMN_NAME, FKTABLE_CATALOG, FKTABLE_SCHEMA, FKTABLE_NAME, FKCOLUMN_NAME").query[CrossReference].to[List].transact(xa)

  protected[reader] def readTables(xa: doobie.Transactor[IO]): IO[List[Table]] =
    (fr0"SELECT TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME, TABLE_TYPE, STORAGE_TYPE, SQL, REMARKS, LAST_MODIFICATION, ID, TYPE_NAME, TABLE_CLASS, ROW_COUNT_ESTIMATE FROM INFORMATION_SCHEMA.TABLES" ++
      schema.map(s => fr0" WHERE TABLE_SCHEMA=$s").getOrElse(fr0"") ++ fr0" ORDER BY TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME").query[Table].to[List].transact(xa)
}
