...
import fr.loicknuchel.scalargs.ArgError.InvalidEnumValue
...
private[gen] object CliConf {
  ...
  private val genReader: Reader[GenConf] =
    Reader.arg(0, "command").inEnum("gen").and(
      readerReader,
      writerReader
    ).map { case (_, r, w) => GenConf(r, w) }
  private val helpReader: Reader[HelpConf] = Reader.hasFlag("help").map(_ => HelpConf())
